...
def list_my_followups():
    ...
    # Incluir dados do aluno em cada follow-up
    followups_data = []
    for f in pag.items:
        ...
        followups_data.append(followup_dict)
...
def admin_list_followups():
    ...
    revisado = request.args.get('revisado')  # 'true'|'false' ou None
    date_from = request.args.get('date_from')  # YYYY-MM-DD
    date_to = request.args.get('date_to')
    search = request.args.get('search')  # busca no texto da atividade
    ...
    if revisado is not None:
        if revisado.lower() in ('true', '1', 'sim', 's'):
            query = query.filter_by(revisado=True)
        elif revisado.lower() in ('false', '0', 'nao', 'não', 'n'):
            query = query.filter_by(revisado=False)
    if date_from:
        try:
    ...
    if date_to:
        try:
            ...
            pass
    if search:
        query = query.filter(FollowUp.atividade_texto.contains(search))
